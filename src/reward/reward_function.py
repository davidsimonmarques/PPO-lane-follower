...
class RewardFunction:
    def __init__(self, config: Dict):
        ...
        self.stuck_penalty = config.get("stuck_penalty", 100.0)
        ...
        self.brake_penalty = config.get("brake_penalty", 0.2)


    def compute(self, observation: Dict, action: list, done: bool, info: Dict = None) -> float:
        ...
        is_stuck = observation.get("stuck", False)
        ...
        # 1. Bônus e Penalidades Terminais
        if is_success: 
        ...
        if is_collision:
            # Aplicamos a mesma penalidade grave para colisões
            return -self.offroad_penalty
            
        if is_stuck:
            return -self.stuck_penalty
            
        # Fallback de segurança: se terminou por outro motivo que não listado
        if done:
            return 0.0
        ...
        # Penaliza esterçamento brusco do volante
        steering_action = abs(action[1]) # steer é action[1]
        steering_penalty_weight = 0.06
        reward -= steering_penalty_weight * (steering_action ** 2) # Ao quadrado penaliza mais os extremos
        
        # Penalidade por usar o freio (quando a ação de throttle_brake é negativa)
        # brake_action = max(0.0, -action[0])
        # reward -= self.brake_penalty * brake_action
